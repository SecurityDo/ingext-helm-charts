...
###############################################################################
# Ingext GKE Status Checker
#
# Checks the status of Ingext installation on GKE, including cluster status,
...
Usage:
  ./status-ingext-gke.sh [options]

Optional options:
  --namespace <name>               Kubernetes namespace (default: ingext)
  --project <project-id>            GCP project ID (for cluster status)
  --region <region>                GCP region (for cluster status)
  --cluster-name <name>            GKE cluster name (for cluster status)
...
if [[ -n "${PROJECT_ID:-}" ]] && [[ -n "${REGION:-}" ]] && [[ -n "${CLUSTER_NAME:-}" ]]; then
  need gcloud
  log "GKE Cluster Status"
  if gcloud auth list --filter=status:ACTIVE --format="value(account)" >/dev/null 2>&1; then
    CLUSTER_STATUS=$(gcloud container clusters describe "$CLUSTER_NAME" --region="$REGION" --project="$PROJECT_ID" --format="value(status)" 2>/dev/null || echo "Unknown")
    ...
    echo "Not logged in to GCP, skipping cluster status check"
  fi
fi
