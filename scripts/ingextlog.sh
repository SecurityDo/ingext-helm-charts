...
case "$service_name" in
    "api")
        label="ingext.io/app=api"
        ;;
    "stream")
        label="ingext.io/app=platform"
        ;;
    "platform")
        label="ingext.io/app=platform"
        ;;	
    "mgr")
        label="ingext.io/app=lake-mgr"
        ;;
    "worker")
        label="ingext.io/app=lake-worker"
        ;;
    "search")
        label="ingext.io/app=search-service"
        ;;
    *)
        # Default catch-all for unknown services
        echo "Error: unknown service '$service_name'"
        echo "Supported services: api, stream, mgr, worker, search"
        exit 1
        ;;
esac
