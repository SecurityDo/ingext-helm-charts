...
export type InstallationState = {
  ...
  details: {
    ...
    iamPolicyExists: boolean;
    karpenterInstalled: boolean;
    namespaceExists: boolean;
    coreChartsInstalled: string[]; // List of installed core charts
    streamChartsInstalled: string[]; // List of installed stream charts
    datalakeChartsInstalled: string[]; // List of installed datalake charts
  };
};
 ...
 */
export async function checkInstallationState(
  ...
  if (s3Bucket) {
    const s3Check = await aws(
      ["s3api", "head-bucket", "--bucket", s3Bucket],
      profile,
      region
    );
    ...
    const policyCheck = await aws(
      ["iam", "list-policies", "--query", `Policies[?PolicyName=='${policyName}'].Arn`, "--output", "text"],
      profile,
      region
    );
    ...
    if (helmList.ok) {
      try {
        ...
        state.details.coreChartsInstalled = releases
          .filter((r: any) => coreCharts.includes(r.name) && r.status === "deployed")
          .map((r: any) => r.name);
    ...
    if (helmList.ok) {
      try {
        ...
        state.details.streamChartsInstalled = releases
          .filter((r: any) => streamCharts.includes(r.name) && r.status === "deployed")
          .map((r: any) => r.name);
 ...
 */
export function displayInstallationState(state: InstallationState): string {
  const lines: string[] = [];
