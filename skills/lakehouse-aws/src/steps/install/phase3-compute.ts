import { setupKarpenter, checkKarpenterInstalled, checkKarpenterReady, repairKarpenter } from "../../tools/karpenter.js";
...
export async function runPhase3Compute(
  ...
  if (installCheck.needsRepair) {
    ...
    if (!repairResult.ok) {
      blockers.push({
        ...
        message: `Failed to repair Karpenter installation: ${repairResult.stderr.split("\n").slice(-5).join("\n")}`,
      });
      return { ok: false, evidence, blockers };
    }
    ...
    if (!setupResult.ok) {
      blockers.push({
        ...
        message: `Karpenter setup failed: ${setupResult.stderr.split("\n").slice(-10).join("\n")}`,
      });
      return { ok: false, evidence, blockers };
    }
  ...
  if (!readyCheck.ready) {
    ...
    // Create actionable blocker with events
