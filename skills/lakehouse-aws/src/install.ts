...
import { InstallInput } from "./schema.js";
...
import { runPhase4CoreServices, Phase4Evidence } from "./steps/install/phase4-core-services.js";
import { runPhase5Stream, Phase5Evidence } from "./steps/install/phase5-stream.js";
import { runPhase6Datalake, Phase6Evidence } from "./steps/install/phase6-datalake.js";
import { runPhase7Ingress, Phase7Evidence } from "./steps/install/phase7-ingress.js";
import { readEnvFile } from "./tools/file.js";
import { checkInstallationState, displayInstallationState } from "./steps/checks/installation-state.js";
import { checkDockerAvailable } from "./steps/checks.js";
import { getExecMode } from "./tools/shell.js";

export type InstallResult = {
  status: "needs_input" | "completed_phase" | "blocked_phase" | "completed" | "error";
  ...
  evidence?: 
    ...
    | { phase1: Phase1Evidence; phase2: Phase2Evidence; phase3: Phase3Evidence; phase4: Phase4Evidence }
    | { phase1: Phase1Evidence; phase2: Phase2Evidence; phase3: Phase3Evidence; phase4: Phase4Evidence; phase5: Phase5Evidence }
    | { phase1: Phase1Evidence; phase2: Phase2Evidence; phase3: Phase3Evidence; phase4: Phase4Evidence; phase5: Phase5Evidence; phase6: Phase6Evidence }
    | { phase1: Phase1Evidence; phase2: Phase2Evidence; phase3: Phase3Evidence; phase4: Phase4Evidence; phase5: Phase5Evidence; phase6: Phase6Evidence; phase7: Phase7Evidence };
  blockers?: Array<{ code: string; message: string }>;
  next?: { action: "install" | "fix" | "stop"; phase?: string };
};

export function renderInstallPlan(env: Record<string, string>, colors?: { GREEN: string; NC: string }): string {
  ...
  const highlight = (value: string) => `${GREEN}${value}${NC}`;
...
export async function runInstall(
  input: { approve: boolean; env: Record<string, string>; force?: boolean; namespace?: string; envFile?: string; verbose?: boolean },
  ...
  if (!phase4Result.ok) {
    ...
    const isDependencyIssue = phase4Result.blockers.some(
      b => b.code === "NO_NODES_AVAILABLE" || 
           b.code === "NO_READY_NODES" || 
  ...
  if (!phase5Result.ok) {
    // Determine if this is a blocker (platform health) or error (fatal)
    const isDependencyIssue = phase5Result.blockers.some(
      b => b.code === "PLATFORM_UNHEALTHY" || 
  ...
  if (!phase6Result.ok) {
    // Determine if this is a blocker (dependencies not met) or error (fatal)
    const isDependencyIssue = phase6Result.blockers.some(
      b => b.code === "STREAM_PODS_NOT_READY" ||
