 ...
 */
export async function listNodegroups(clusterName, awsProfile, awsRegion) {
    const res = await aws(["eks", "list-nodegroups", "--cluster-name", clusterName, "--query", "nodegroups", "--output", "json"], awsProfile, awsRegion);
    if (!res.ok) {
    ...
    try {
        ...
        return { ok: true, nodegroups: Array.isArray(nodegroups) ? nodegroups : [] };
    }
    catch {
        return { ok: false, nodegroups: [], error: "Failed to parse nodegroups list" };
    }
}
/**
 * Get node group status
 */
export async function describeNodegroup(clusterName, nodegroupName, awsProfile, awsRegion) {
    const res = await aws(["eks", "describe-nodegroup", "--cluster-name", clusterName, "--nodegroup-name", nodegroupName, "--query", "nodegroup.status", "--output", "text"], awsProfile, awsRegion);
    if (!res.ok) {
        const isNotFound = res.stderr.includes("ResourceNotFoundException") ||
            res.stderr.includes("not found");
        return {
 ...
 */
export async function getVpcIdFromCluster(clusterName, awsProfile, awsRegion) {
    const res = await aws(["eks", "describe-cluster", "--name", clusterName, "--region", awsRegion, "--query", "cluster.resourcesVpcConfig.vpcId", "--output", "text"], awsProfile, awsRegion);
 ...
 * Delete a node group
 */
export async function deleteNodegroup(clusterName, nodegroupName, awsProfile, awsRegion) {
    return aws(["eks", "delete-nodegroup", "--cluster-name", clusterName, "--nodegroup-name", nodegroupName], awsProfile, awsRegion);
}
/**
 * Delete EKS cluster using AWS CLI (fallback when eksctl is not available)
 * First deletes all node groups, then deletes the cluster
 */
export async function deleteClusterWithAwsCli(clusterName, awsProfile, awsRegion) {
    ...
    if (nodegroupsResult.ok && nodegroupsResult.nodegroups.length > 0) {
        ...
        while (attempts < maxAttempts && !allDeleted) {
            ...
            if (checkResult.ok === false && checkResult.clusterNotFound) {
                allDeleted = true;
                // Clear the progress line
                process.stderr.write(`\r${" ".repeat(80)}\r`); // Clear line
                console.error(`  ✓ Cluster deleted (node groups automatically deleted)`);
            ...
            if (checkResult.ok && checkResult.nodegroups.length === 0) {
                ...
                const statusChecks = await Promise.all(remaining.map(ng => describeNodegroup(clusterName, ng, awsProfile, awsRegion)));
                const statuses = statusChecks.map((check, i) => check.found ? `${remaining[i]}:${check.status}` : `${remaining[i]}:UNKNOWN`);
                // If all are DELETING or NOT_FOUND, we can proceed (don't need to wait for complete removal)
                const allDeletingOrGone = statusChecks.every(check => !check.found || check.status === "DELETING" || check.status === "NOT_FOUND");
                if (allDeletingOrGone) {
                    // All node groups are deleting or gone - we can proceed with cluster deletion
                    // Node groups will finish deleting asynchronously
                    allDeleted = true;
                    // Clear the progress line
                    process.stderr.write(`\r${" ".repeat(80)}\r`); // Clear line
                    const deletingCount = statusChecks.filter(check => check.found && check.status === "DELETING").length;
                    if (deletingCount > 0) {
                        console.error(`  ✓ All node groups are DELETING (${deletingCount} in progress, will complete asynchronously)`);
                    }
                    else {
                        console.error(`  ✓ All node groups deleted`);
                    }
                    ...
                    if (activeCount > 0) {
                        ...
                        process.stderr.write(`\r   [${minutes}m ${seconds}s] ${deletingCount} node group(s) DELETING: ${statuses.join(", ")}${" ".repeat(20)}`);
                    }
                    else {
    ...
    const deleteResult = await aws(["eks", "delete-cluster", "--name", clusterName], awsProfile, awsRegion);
    return {
        ok: deleteResult.ok,
 ...
 */
export async function listCloudFormationStacks(clusterName, awsProfile, awsRegion) {
 ...
 */
export async function describeCloudFormationStack(stackName, awsProfile, awsRegion) {
    const statusRes = await aws(["cloudformation", "describe-stacks", "--stack-name", stackName, "--query", "Stacks[0].{Status:StackStatus,StatusReason:StackStatusReason,TerminationProtection:TerminationProtection}", "--output", "json"], awsProfile, awsRegion);
    if (!statusRes.ok) {
    ...
    try {
        ...
        const eventsRes = await aws(["cloudformation", "describe-stack-events", "--stack-name", stackName, "--max-items", "10", "--query", "StackEvents[?ResourceStatus=='DELETE_FAILED' || ResourceStatus=='CREATE_FAILED' || ResourceStatus=='UPDATE_FAILED'].{Resource:LogicalResourceId,Status:ResourceStatus,Reason:ResourceStatusReason}", "--output", "json"], awsProfile, awsRegion);
        const events = eventsRes.ok ? JSON.parse(eventsRes.stdout) : [];
        // Get stack resources
        const resourcesRes = await aws(["cloudformation", "describe-stack-resources", "--stack-name", stackName, "--query", "StackResources[?ResourceStatus!='DELETE_COMPLETE'].{Resource:LogicalResourceId,Type:ResourceType,Status:ResourceStatus}", "--output", "json"], awsProfile, awsRegion);
 ...
 */
async function getSubnetIdsFromStack(stackName, awsProfile, awsRegion) {
    const res = await aws(["cloudformation", "describe-stack-resources", "--stack-name", stackName, "--query", "StackResources[?ResourceType=='AWS::EC2::Subnet'].PhysicalResourceId", "--output", "text"], awsProfile, awsRegion);
    if (!res.ok)
        return [];
 ...
 */
async function findNetworkInterfacesInSubnets(subnetIds, awsProfile, awsRegion) {
    if (subnetIds.length === 0)
        return [];
    const res = await aws(["ec2", "describe-network-interfaces", "--filters", `Name=subnet-id,Values=${subnetIds.join(",")}`, "--query", "NetworkInterfaces[*].NetworkInterfaceId", "--output", "text"], awsProfile, awsRegion);
    if (!res.ok)
        return [];
 ...
 */
async function getNetworkInterfaceDetails(eniId, awsProfile, awsRegion) {
    const res = await aws(["ec2", "describe-network-interfaces", "--network-interface-ids", eniId, "--query", "NetworkInterfaces[0].{AttachmentId:Attachment.AttachmentId,Status:Status,Description:Description}", "--output", "json"], awsProfile, awsRegion);
 ...
 */
async function detachNetworkInterface(eniId, attachmentId, awsProfile, awsRegion) {
    const res = await aws(["ec2", "detach-network-interface", "--attachment-id", attachmentId, "--force"], awsProfile, awsRegion);
    if (res.ok) {
 ...
 */
async function deleteNetworkInterfaces(eniIds, awsProfile, awsRegion) {
    const deleted = [];
    const failed = [];
    for (const eniId of eniIds) {
        ...
        // Now delete the ENI
        const res = await aws(["ec2", "delete-network-interface", "--network-interface-id", eniId], awsProfile, awsRegion);
        if (res.ok) {
            deleted.push(eniId);
        }
        else {
            ...
            if (res.stderr.includes("does not exist") || res.stderr.includes("InvalidNetworkInterfaceID.NotFound")) {
                ...
                const retryRes = await aws(["ec2", "delete-network-interface", "--network-interface-id", eniId], awsProfile, awsRegion);
 ...
 */
async function getRouteTableAssociations(subnetIds, awsProfile, awsRegion) {
    if (subnetIds.length === 0)
        return [];
    const associations = [];
    for (const subnetId of subnetIds) {
        // First, get all route tables that have associations with this subnet
        const res = await aws(["ec2", "describe-route-tables", "--filters", `Name=association.subnet-id,Values=${subnetId}`, "--query", "RouteTables[*].[RouteTableId,Associations[?SubnetId==`'${subnetId}'`].RouteTableAssociationId]", "--output", "json"], awsProfile, awsRegion);
        if (res.ok && res.stdout.trim()) {
            try {
 ...
 */
async function disassociateRouteTables(associations, awsProfile, awsRegion) {
    let disassociated = 0;
    let failed = 0;
    for (const assoc of associations) {
        const res = await aws(["ec2", "disassociate-route-table", "--association-id", assoc.associationId], awsProfile, awsRegion);
        if (res.ok) {
            disassociated++;
        }
        else {
 ...
 */
async function getInternetGatewayForVpc(vpcId, awsProfile, awsRegion) {
    const res = await aws(["ec2", "describe-internet-gateways", "--filters", `Name=attachment.vpc-id,Values=${vpcId}`, "--query", "InternetGateways[0].InternetGatewayId", "--output", "text"], awsProfile, awsRegion);
    if (res.ok && res.stdout.trim() && !res.stdout.includes("None")) {
        return res.stdout.trim();
    }
    return null;
 ...
 */
async function detachInternetGateway(igwId, vpcId, awsProfile, awsRegion) {
    const res = await aws(["ec2", "detach-internet-gateway", "--internet-gateway-id", igwId, "--vpc-id", vpcId], awsProfile, awsRegion);
 ...
 */
async function getVpcIdFromStack(stackName, awsProfile, awsRegion) {
    const res = await aws(["cloudformation", "describe-stack-resources", "--stack-name", stackName, "--query", "StackResources[?ResourceType=='AWS::EC2::VPC'].PhysicalResourceId", "--output", "text"], awsProfile, awsRegion);
    if (res.ok && res.stdout.trim()) {
        return res.stdout.trim();
    }
    return null;
 ...
 */
async function getVpcIdFromSubnet(subnetId, awsProfile, awsRegion) {
    const res = await aws(["ec2", "describe-subnets", "--subnet-ids", subnetId, "--query", "Subnets[0].VpcId", "--output", "text"], awsProfile, awsRegion);
    if (res.ok && res.stdout.trim() && !res.stdout.includes("None")) {
        return res.stdout.trim();
    }
    return null;
 ...
 */
async function deleteLoadBalancersInVpc(vpcId, awsProfile, awsRegion, subnetId) {
    let deleted = 0;
    let failed = 0;
    // 1. Check Application and Network Load Balancers (v2)
    const lbRes = await aws(["elbv2", "describe-load-balancers", "--query", "LoadBalancers[].{Arn:LoadBalancerArn,VpcId:VpcId,Subnets:AvailabilityZones[].SubnetId}", "--output", "json"], awsProfile, awsRegion);
        ...
        try {
            ...
            for (const lb of lbs) {
                ...
                if (match) {
                    ...
                    const delRes = await aws(["elbv2", "delete-load-balancer", "--load-balancer-arn", lb.Arn], awsProfile, awsRegion);
                    if (delRes.ok) {
                        deleted++;
                    }
                    else {
    ...
    const classicRes = await aws(["elb", "describe-load-balancers", "--query", "LoadBalancerDescriptions[].{Name:LoadBalancerName,VPC:VPCId,Subnets:Subnets}", "--output", "json"], awsProfile, awsRegion);
    if (classicRes.ok && classicRes.stdout.trim()) {
        try {
            ...
            for (const lb of lbs) {
                ...
                if (match) {
                    const delRes = await aws(["elb", "delete-load-balancer", "--load-balancer-name", lb.Name], awsProfile, awsRegion);
                    if (delRes.ok) {
                        deleted++;
                    }
                    else {
 ...
 */
async function forceCleanupSubnet(subnetId, subnetLogicalName, awsProfile, awsRegion) {
    const dependencies = [];
    let cleaned = true;
    ...
    const natGatewayRes = await aws(["ec2", "describe-nat-gateways", "--filter", `Name=subnet-id,Values=${subnetId}`, "--query", "NatGateways[?State!='deleted'].NatGatewayId", "--output", "text"], awsProfile, awsRegion);
    if (natGatewayRes.ok && natGatewayRes.stdout.trim()) {
        ...
        if (natGatewayIds.length > 0) {
            ...
            for (const natId of natGatewayIds) {
                // Delete NAT Gateway
                await aws(["ec2", "delete-nat-gateway", "--nat-gateway-id", natId], awsProfile, awsRegion);
            }
    ...
    const vpcEndpointRes = await aws(["ec2", "describe-vpc-endpoints", "--filters", `Name=subnet-id,Values=${subnetId}`, "--query", "VpcEndpoints[?State!='deleted'].VpcEndpointId", "--output", "text"], awsProfile, awsRegion);
    if (vpcEndpointRes.ok && vpcEndpointRes.stdout.trim()) {
        ...
        if (vpcEndpointIds.length > 0) {
            ...
            for (const endpointId of vpcEndpointIds) {
                await aws(["ec2", "delete-vpc-endpoints", "--vpc-endpoint-ids", endpointId], awsProfile, awsRegion);
            }
            await new Promise(resolve => setTimeout(resolve, 5000));
        }
    }
 ...
 */
export async function deleteCloudFormationStack(stackName, awsProfile, awsRegion, options) {
    ...
    if (options?.cleanupDependencies) {
        ...
        if (stackDetails.ok && (stackDetails.status === "DELETE_FAILED" || stackDetails.status === "DELETE_IN_PROGRESS")) {
            ...
            const stuckOrFailedSubnets = resources.filter((r) => r.Type === "AWS::EC2::Subnet" && (r.Status === "DELETE_FAILED" || r.Status === "DELETE_IN_PROGRESS"));
            ...
            const stuckOrFailedGatewayAttachment = resources.find((r) => r.Type === "AWS::EC2::VPCGatewayAttachment" && (r.Status === "DELETE_FAILED" || r.Status === "DELETE_IN_PROGRESS"));
            ...
            const igwResource = resources.find((r) => r.Type === "AWS::EC2::InternetGateway" && r.Status === "CREATE_COMPLETE");
            if (igwResource) {
                const vpcId = await getVpcIdFromStack(stackName, awsProfile, awsRegion);
                if (vpcId) {
    ...
    const res = await aws(["cloudformation", "delete-stack", "--stack-name", stackName], awsProfile, awsRegion);
    return {
        ok: res.ok,
        stderr: res.stderr,
        stdout: res.stdout,
 ...
 */
async function checkStackDeleted(stackName, awsProfile, awsRegion, stuckThresholdMinutes = 5) {
    const checkRes = await aws(["cloudformation", "describe-stacks", "--stack-name", stackName, "--query", "Stacks[0].StackStatus", "--output", "text"], awsProfile, awsRegion);
    if (!checkRes.ok) {
    ...
    // If stack is in DELETE_IN_PROGRESS, get detailed resource information
    if (status === "DELETE_IN_PROGRESS") {
        // Get resources with their types, statuses, and physical IDs
        const resourcesRes = await aws(["cloudformation", "describe-stack-resources", "--stack-name", stackName, "--query", "StackResources[?ResourceStatus=='DELETE_IN_PROGRESS' || ResourceStatus=='DELETE_FAILED'].{Resource:LogicalResourceId,Type:ResourceType,Status:ResourceStatus,PhysicalId:PhysicalResourceId}", "--output", "json"], awsProfile, awsRegion);
        ...
        if (resourcesRes.ok && resourcesRes.stdout.trim()) {
            try {
                ...
                for (const resource of resources) {
                    ...
                    if (resourceType === "AWS::EC2::Subnet" && resource.Status === "DELETE_IN_PROGRESS") {
                    ...
                    if (resourceType === "AWS::EC2::VPCGatewayAttachment" && resource.Status === "DELETE_IN_PROGRESS") {
                        const vpcId = await getVpcIdFromStack(stackName, awsProfile, awsRegion);
                        if (vpcId) {
                            const igwId = await getInternetGatewayForVpc(vpcId, awsProfile, awsRegion);
                            if (igwId) {
    ...
    // If stack is in DELETE_FAILED, it failed to delete and needs manual intervention
 ...
 */
export async function waitForStacksDeleted(stackNames, awsProfile, awsRegion, maxWaitMinutes = 10) {
    ...
    const stuckResourceTimestamps = new Map();
    ...
    while (attempts < maxAttempts && remainingStacks.length > 0) {
        ...
        const checkPromises = remainingStacks.map(stackName => checkStackDeleted(stackName, awsProfile, awsRegion, stuckThresholdMinutes).then(result => ({ stackName, ...result })));
        const results = await Promise.all(checkPromises);
        // Separate deleted from remaining and collect resource info
        const stillRemaining = [];
        const resourceStatuses = [];
        const failedStacks = [];
        const stacksNeedingCleanup = [];
 ...
 */
export async function waitForStackDeleted(stackName, awsProfile, awsRegion, maxWaitMinutes = 10) {
