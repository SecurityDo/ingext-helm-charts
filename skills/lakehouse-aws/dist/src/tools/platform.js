...
export async function checkPlatformHealth(profile, region) {
    const blockers = [];
    const result = {
        healthy: true,
    ...
    const corednsCheck = await kubectl(["get", "deployment", "coredns", "-n", "kube-system", "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    if (corednsCheck.ok) {
        try {
