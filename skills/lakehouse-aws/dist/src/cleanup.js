...
export async function runCleanup(input) {
    ...
    for (const release of HELM_RELEASES) {
        try {
            // Try namespace first
            const nsResult = await helm(["uninstall", release, "-n", namespace], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
            if (nsResult.ok || nsResult.stderr.includes("not found")) {
                ...
                // Try kube-system
                const kubeSystemResult = await helm(["uninstall", release, "-n", "kube-system"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
    ...
    // Get account ID
    const accountIdResult = await aws(getExecMode(), ["sts", "get-caller-identity", "--query", "Account", "--output", "text"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
    ...
    try {
        const volumesResult = await aws(getExecMode(), [
            "ec2",
            "describe-volumes",
            "--region",
            env.AWS_REGION,
        ...
        if (volumesResult.ok && volumesResult.stdout.trim()) {
            ...
            for (const volumeId of volumeIds) {
                try {
                    const deleteResult = await aws(getExecMode(), ["ec2", "delete-volume", "--volume-id", volumeId, "--region", env.AWS_REGION], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
                    if (deleteResult.ok) {
