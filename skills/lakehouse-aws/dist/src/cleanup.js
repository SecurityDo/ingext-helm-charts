...
import { aws, describeCluster, deleteClusterWithAwsCli, listCloudFormationStacks, deleteCloudFormationStack, waitForStacksDeleted } from "./tools/aws.js";
...
function renderCleanupPlan(env) {
    return `
${"═".repeat(80)}
...
export async function runCleanup(input) {
    ...
    for (const release of HELM_RELEASES) {
        try {
            ...
            const uninstallResult = await helm(["uninstall", release, "-n", releaseNamespace, "--wait", "--timeout", "5m"], helmEnv);
            if (uninstallResult.ok || uninstallResult.stderr.includes("not found")) {
                ...
                const fallbackResult = await helm(["uninstall", release, "-n", releaseNamespace], helmEnv);
                if (fallbackResult.ok || fallbackResult.stderr.includes("not found")) {
                    ...
                    const otherNsResult = await helm(["uninstall", release, "-n", otherNamespace], helmEnv);
    ...
    if (evidence.helmReleases.uninstalled.length > 0) {
        ...
        while (attempts < maxAttempts && !allGone) {
            ...
            const helmListResult = await helm(["list", "-a", "-n", namespace, "-o", "json"], helmEnv);
            if (helmListResult.ok) {
                try {
                    const releases = JSON.parse(helmListResult.stdout);
                    const existingReleases = releases.map((r) => r.name);
    ...
    try {
        ...
        const deploymentsCheck = await kubectl(["get", "deployments", "-n", namespace, "-o", "json"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
            ...
            try {
                ...
                if (deployments.length > 0) {
                    ...
                    for (const deployment of deployments) {
                        await kubectl(["delete", "deployment", deployment.metadata.name, "-n", namespace, "--ignore-not-found=true"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
                    }
                }
            }
            catch (e) {
        ...
        const statefulsetsCheck = await kubectl(["get", "statefulsets", "-n", namespace, "-o", "json"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
            ...
            try {
                ...
                if (statefulsets.length > 0) {
                    ...
                    for (const sts of statefulsets) {
                        await kubectl(["delete", "statefulset", sts.metadata.name, "-n", namespace, "--ignore-not-found=true"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
                    }
                }
            }
            catch (e) {
        ...
        const podsCheck = await kubectl(["get", "pods", "-n", namespace, "-o", "json"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
        if (podsCheck.ok) {
            try {
                ...
                if (activePods.length > 0) {
                    ...
                    for (const pod of activePods) {
                        await kubectl(["delete", "pod", pod.metadata.name, "-n", namespace, "--ignore-not-found=true"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
                    }
                }
            }
            catch (e) {
    ...
    if (!clusterCheck.found && clusterCheck.status === "NOT_FOUND") {
        ...
        try {
            ...
            if (clusterResult.ok) {
                ...
                const errorText = clusterResult.stderr || clusterResult.stdout || "";
                // Get exit code from the result (eksctl returns ShellResult which has exitCode)
                const exitCode = clusterResult.exitCode ?? (clusterResult.ok ? 0 : 1);
                // Check if eksctl command itself is not found (exit code 127)
                if (exitCode === 127 || errorText.includes("Command not found") || errorText.includes("eksctl not found")) {
                    // Fallback to AWS CLI for cluster deletion
                    console.error(`  ⚠️  eksctl not found. Using AWS CLI to delete cluster instead...`);
                    console.error(`  This will delete node groups first, then the cluster.`);
                    try {
                        const awsCliResult = await deleteClusterWithAwsCli(env.CLUSTER_NAME, env.AWS_PROFILE, env.AWS_REGION);
                        if (awsCliResult.ok) {
                            // Wait and verify deletion
                            console.error(`  ✓ Cluster deletion initiated`);
                            console.error(`  ⏳ Waiting for cluster deletion to complete...`);
                            console.error(`     ⚠️  This may take up to 15 minutes. Cluster is being deleted asynchronously by AWS.`);
                            console.error(`     Progress will be shown below (updates every 15 seconds):`);
                            let deleted = false;
                            ...
                            evidence.cluster = { deleted: false, error: `AWS CLI cluster deletion failed: ${awsCliResult.stderr}` };
                            blockers.push({
                                code: "CLUSTER_DELETE_AWSCLI_FAILED",
                                message: `Failed to delete cluster using AWS CLI: ${awsCliResult.stderr}`,
                            });
                            console.error(`  ⚠️  Failed to delete cluster using AWS CLI: ${awsCliResult.stderr}`);
                        }
                    }
                    catch (err) {
                        evidence.cluster = { deleted: false, error: `Error deleting cluster with AWS CLI: ${err}` };
                        blockers.push({
                            code: "CLUSTER_DELETE_AWSCLI_ERROR",
                            message: `Error deleting cluster with AWS CLI: ${err}`,
                        });
                        console.error(`  ⚠️  Error deleting cluster with AWS CLI: ${err}`);
                    }
                }
                // Check if error indicates cluster doesn't exist
                else if (errorText.includes("not found") || errorText.includes("does not exist") ||
                    ...
                    if (!verifyCheck.found && verifyCheck.status === "NOT_FOUND") {
                        ...
                        // Cluster still exists despite error message
                        evidence.cluster = { deleted: false, error: `eksctl reported cluster not found but cluster still exists with status: ${verifyCheck.status}` };
                        blockers.push({
                            code: "CLUSTER_DELETE_FAILED",
                            message: `eksctl delete failed but cluster still exists. Status: ${verifyCheck.status}. You may need to manually delete it.`,
                        });
                        console.error(`  ⚠️  Cluster deletion failed. Cluster still exists with status: ${verifyCheck.status}`);
                    }
                }
                else {
                    // Other error - verify cluster status
                    const verifyCheck = await describeCluster(env.CLUSTER_NAME, env.AWS_PROFILE, env.AWS_REGION);
                    evidence.cluster = { deleted: false, error: clusterResult.stderr || "Unknown error" };
                    blockers.push({
                        code: "CLUSTER_DELETE_FAILED",
                        message: `Failed to delete cluster: ${clusterResult.stderr || "Unknown error"}. Cluster status: ${verifyCheck.found ? verifyCheck.status : "UNKNOWN"}`,
                    });
                    console.error(`  ⚠️  Failed to delete cluster: ${clusterResult.stderr || "Unknown error"}`);
                    if (verifyCheck.found) {
                        console.error(`  ⚠️  Cluster still exists with status: ${verifyCheck.status}`);
                    }
                }
            }
        }
                ...
                const deletePromises = sortedStacks.map(async (stackName) => {
                    ...
                    if (!deleteResult.ok) {
                        ...
                        if (deleteResult.stderr.includes("TerminationProtection is enabled")) {
                            ...
                            const disableResult = await aws(["cloudformation", "update-termination-protection", "--stack-name", stackName, "--no-enable-termination-protection"], env.AWS_PROFILE, env.AWS_REGION);
                            if (disableResult.ok) {
                                // Retry deletion
    ...
    // Get account ID
    const accountIdResult = await aws(["sts", "get-caller-identity", "--query", "Account", "--output", "text"], env.AWS_PROFILE, env.AWS_REGION);
    ...
    try {
        const volumesResult = await aws([
            ...
            "--output",
            "text",
        ], env.AWS_PROFILE, env.AWS_REGION);
        if (volumesResult.ok && volumesResult.stdout.trim()) {
            ...
            for (const volumeId of volumeIds) {
                try {
                    const deleteResult = await aws(["ec2", "delete-volume", "--volume-id", volumeId, "--region", env.AWS_REGION], env.AWS_PROFILE, env.AWS_REGION);
                    if (deleteResult.ok) {
