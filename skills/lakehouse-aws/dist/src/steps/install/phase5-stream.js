...
export async function runPhase5Stream(env, options = {}) {
    ...
    const platformHealth = await checkPlatformHealth(env.AWS_PROFILE, env.AWS_REGION);
    ...
    const configResult = await upgradeInstall("ingext-community-config", "oci://public.ecr.aws/ingext/ingext-community-config", namespace, { siteDomain: siteDomain }, { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
    ...
    const initResult = await upgradeInstall("ingext-community-init", "oci://public.ecr.aws/ingext/ingext-community-init", namespace, undefined, { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
    ...
    if (!waitResult.ok) {
        // Capture diagnostics
        const podsCheck = await kubectl(["get", "pods", "-n", namespace, "-o", "wide"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
        ...
        const eventsResult = await kubectl(["get", "events", "-n", namespace, "--sort-by=.lastTimestamp", "-o", "json"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
        if (eventsResult.ok) {
            try {
        ...
        for (const pod of notReadyPods) {
            const describeResult = await kubectl(["describe", "pod", pod.name, "-n", namespace], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
    ...
    // All pods are ready
    const finalPodsCheck = await kubectl(["get", "pods", "-n", namespace, "-o", "json"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
