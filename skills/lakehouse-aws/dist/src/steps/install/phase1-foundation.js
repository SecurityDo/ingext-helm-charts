...
export async function runPhase1Foundation(env, options) {
    ...
    if (clusterStatus.status !== "ACTIVE") {
        ...
        if (!waitResult.ok) {
            if (verbose)
                console.error(`❌ Cluster did not become ACTIVE: ${waitResult.error}`);
            blockers.push({
    ...
    if (verbose)
        console.error(`\n⏳ Updating kubeconfig...`);
    const kubeconfigResult = await aws(["eks", "update-kubeconfig", "--region", region, "--name", clusterName, "--alias", clusterName], profile, region);
    evidence.eks.kubeconfigUpdated = kubeconfigResult.ok;
    if (!kubeconfigResult.ok) {
        if (verbose)
            console.error(`❌ Failed to update kubeconfig: ${kubeconfigResult.stderr.substring(0, 200)}`);
        blockers.push({
    ...
    if (verbose)
        console.error(`   Installing kube-proxy...`);
    ...
    if (kubeProxyAddon.ok) {
        ...
        if (verbose)
            console.error(`   ✓ kube-proxy installed`);
    }
    else if (verbose) {
    ...
    if (ebsCsiAddon.ok) {
        ...
        if (verbose)
            console.error(`✓ EBS CSI driver addon installed`);
    }
    else {
        if (verbose)
            console.error(`⚠️  EBS CSI driver addon may already exist`);
    }
    // 9. Create IAM role for EBS CSI driver
    if (verbose)
        console.error(`⏳ Creating IAM role for EBS CSI driver...`);
    ...
    if (!roleCheck.ok) {
        ...
        if (!roleResult.ok && !roleResult.stderr.includes("EntityAlreadyExists")) {
            if (verbose)
                console.error(`⚠️  Failed to create EBS CSI driver role: ${roleResult.stderr.substring(0, 200)}`);
            // Continue anyway - maybe role exists but getRole failed
        }
        else {
            if (verbose)
                console.error(`✓ Created IAM role: ${ebsRoleName}`);
        }
        ...
        if (!attachResult.ok && !attachResult.stderr.includes("already attached")) {
            if (verbose)
                console.error(`⚠️  Failed to attach policy to role: ${attachResult.stderr.substring(0, 200)}`);
        }
        else {
            if (verbose)
                console.error(`✓ Attached policy: AmazonEBSCSIDriverPolicy`);
        }
    }
    else {
        if (verbose)
            console.error(`✓ IAM role already exists: ${ebsRoleName}`);
    }
    // 10. NOW create pod identity association (service account exists now)
    if (verbose)
        console.error(`⏳ Creating pod identity association for EBS CSI driver...`);
    ...
    const storageClassResult = await upgradeInstall("ingext-aws-gp3", "oci://public.ecr.aws/ingext/ingext-aws-gp3", "kube-system", { "storageClass.isDefaultClass": "true" }, { AWS_PROFILE: profile, AWS_REGION: region });
    ...
    if (verbose)
        console.error(`   Installing aws-mountpoint-s3-csi-driver...`);
    const s3CsiResult = await aws(["eks", "create-addon", "--cluster-name", clusterName, "--addon-name", "aws-mountpoint-s3-csi-driver", "--region", region], profile, region);
    ...
    if (s3CsiResult.ok || s3CsiResult.stderr.includes("already exists")) {
        ...
        if (verbose)
            console.error(`   ✓ aws-mountpoint-s3-csi-driver installed`);
    }
    else if (verbose) {
