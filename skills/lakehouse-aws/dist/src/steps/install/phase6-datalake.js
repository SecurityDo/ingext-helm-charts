...
async function analyzeCrashLoop(podName, namespace, env) {
    ...
    const previousLogsResult = await kubectl(["logs", "-n", namespace, podName, "--all-containers", "--previous", "--tail=200"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
    // Fallback to current logs if previous not available
    const logsResult = previousLogsResult.ok
        ? previousLogsResult
        : await kubectl(["logs", "-n", namespace, podName, "--all-containers", "--tail=200"], { AWS_PROFILE: env.AWS_PROFILE, AWS_REGION: env.AWS_REGION });
    ...
    if (logs.includes("connection refused") ||
    ...
    if (logs.includes("no space left") ||
        logs.includes("PVC") ||
    ...
    if ((logs.includes("secrets") || logs.includes("configmaps")) &&
...
export async function runPhase6Datalake(env, options = {}) {
    ...
    if (!bucketCheck.exists) {
        ...
        if (!podIdentityCheck.ok) {
            const saCheck = await kubectl(["get", "serviceaccount", serviceAccountName, "-n", namespace, "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
            if (saCheck.ok) {
                try {
        ...
        if (!karpenterReady.ready) {
            ...
            if (eventsResult.ok) {
                try {
                    ...
                    const failedScheduling = recentEvents.filter((e) => e.reason === "FailedScheduling" ||
                    ...
                    if (failedScheduling.length > 0) {
                        schedulingIssues = failedScheduling
                            .map((e) => `${e.lastTimestamp || ""} ${e.reason || ""} ${e.message || ""}`)
                            .join("\n");
                    }
                }
                catch (e) {
    ...
    const lakeConfigResult = await upgradeInstall("ingext-lake-config", "oci://public.ecr.aws/ingext/ingext-lake-config", namespace, {
        storageType: "s3",
    ...
    const cmCheck = await kubectl(["get", "configmaps", "-n", namespace, "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    if (cmCheck.ok) {
        try {
            const cmData = JSON.parse(cmCheck.stdout);
            const cms = cmData.items || [];
            const lakeConfigMaps = cms.filter((cm) => cm.metadata?.name?.toLowerCase().includes("lake"));
    ...
    const secretCheck = await kubectl(["get", "secrets", "-n", namespace, "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    if (secretCheck.ok) {
        try {
            const secretData = JSON.parse(secretCheck.stdout);
            const secrets = secretData.items || [];
            const lakeSecrets = secrets.filter((s) => s.metadata?.name?.toLowerCase().includes("lake"));
            // Log findings (non-blocking)
        }
        catch (e) {
            // Ignore parse errors
    ...
    const nodepoolsCheck = await kubectl(["get", "nodepools", "-A", "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    ...
    if (!mergePoolVerified || !searchPoolVerified) {
        const provisionersCheck = await kubectl(["get", "provisioners", "-A", "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    ...
    const ec2NodeClassCheck = await kubectl(["get", "ec2nodeclasses", "-A", "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    ...
    const rbacSecretsCheck = await kubectl(["auth", "can-i", "get", "secrets", "-n", namespace, "--as", `system:serviceaccount:${namespace}:${serviceAccountName}`], { AWS_PROFILE: profile, AWS_REGION: region });
    const rbacConfigMapsCheck = await kubectl(["auth", "can-i", "get", "configmaps", "-n", namespace, "--as", `system:serviceaccount:${namespace}:${serviceAccountName}`], { AWS_PROFILE: profile, AWS_REGION: region });
    ...
    const s3LakeResult = await upgradeInstall("ingext-s3-lake", "oci://public.ecr.aws/ingext/ingext-s3-lake", namespace, {
    ...
    const jobsCheck = await kubectl(["get", "jobs", "-n", namespace, "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    if (jobsCheck.ok) {
        try {
            const jobsData = JSON.parse(jobsCheck.stdout);
            const jobs = jobsData.items || [];
            const s3LakeJobs = jobs.filter((job) => job.metadata?.name?.toLowerCase().includes("s3") ||
    ...
    const eventsCheck = await kubectl(["get", "events", "-n", namespace, "--sort-by=.lastTimestamp", "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
    if (eventsCheck.ok) {
        try {
            const eventsData = JSON.parse(eventsCheck.stdout);
            const events = eventsData.items || [];
            const recentFailures = events.slice(-10).filter((e) => e.type === "Warning" || e.reason?.includes("Failed"));
            // Log findings (non-blocking)
        }
        catch (e) {
            // Ignore parse errors
    ...
    if (!waitResult.ok) {
        // Capture diagnostics
        const podsCheck = await kubectl(["get", "pods", "-n", namespace, "-o", "wide"], { AWS_PROFILE: profile, AWS_REGION: region });
        ...
        const eventsResult = await kubectl(["get", "events", "-n", namespace, "--sort-by=.lastTimestamp", "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
        if (eventsResult.ok) {
            try {
                const eventsData = JSON.parse(eventsResult.stdout);
                const events = eventsData.items || [];
                evidence.pods.eventsTail = events
                    .slice(-25)
                    .map((e) => `${e.lastTimestamp || ""} ${e.type || ""} ${e.reason || ""} ${e.message || ""}`)
                    .join("\n");
            }
            catch (e) {
                // Ignore parse errors
        ...
        for (const pod of notReadyPods) {
            const describeResult = await kubectl(["describe", "pod", pod.name, "-n", namespace], { AWS_PROFILE: profile, AWS_REGION: region });
            if (describeResult.ok) {
                // Extract relevant sections (Events, Conditions)
                const describeLines = describeResult.stdout.split("\n");
                const eventsStart = describeLines.findIndex((l) => l.includes("Events:"));
                const conditionsStart = describeLines.findIndex((l) => l.includes("Conditions:"));
                let relevantOutput = "";
        ...
        const crashLoopPods = evidence.pods.notReady.filter(p => p.status === "CrashLoopBackOff" || p.reason?.includes("CrashLoopBackOff"));
        const otherNotReady = evidence.pods.notReady.filter(p => p.status !== "Pending" && p.status !== "CrashLoopBackOff");
        let classification = "";
    ...
    // All pods are ready
    const finalPodsCheck = await kubectl(["get", "pods", "-n", namespace, "-o", "json"], { AWS_PROFILE: profile, AWS_REGION: region });
