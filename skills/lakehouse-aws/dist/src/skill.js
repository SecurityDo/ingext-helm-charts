...
export async function runPreflight(input) {
    ...
    if (!certArn) {
        ...
        if (!certDescribe.ok) {
            ...
            const domainCovered = certSubjectAltNames.some((san) => {
                // Exact match
                if (san === siteDomain)
                    return true;
                // Wildcard match
    ...
    if (!input.readiness.hasBilling)
        blockers.push({ code: "NO_BILLING", message: "Billing must be enabled to create EKS/S3 resources." });
    if (!input.readiness.hasAdmin)
        blockers.push({ code: "NO_ADMIN", message: "Admin permissions are required (IAM/VPC/EKS)." });
    if (!input.readiness.hasDns)
        blockers.push({ code: "NO_DNS", message: `You must control DNS for ${siteDomain}.` });
