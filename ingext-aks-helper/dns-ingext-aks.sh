...
###############################################################################
# Ingext AKS DNS Helper
#
# Helps with DNS configuration and verification for Ingext on AKS.
# Gets the ingress public IP, checks DNS resolution, and monitors
# certificate challenge status.
#
# Usage:
#   ./dns-ingext-aks.sh --domain ingext.example.com [--namespace ingext]
#
# Requirements:
#   - kubectl
...
if [[ "$DNS_READY" == "0" ]]; then
  ...
  echo "     ./dns-ingext-aks.sh --domain $SITE_DOMAIN --namespace $NAMESPACE --wait"
else
